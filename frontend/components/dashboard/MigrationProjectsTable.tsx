...
const PROJECTS: Project[] = [
  ...
  { name: 'Customer Master Conversion Demo', object: 'CUSTOMER', source: 'Legacy EBS R12', method: 'FBDI', batches: 1 },
  ...
  return (
    <div className="flex-1 rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
        <h2 className="text-[15px] font-semibold text-slate-900">Migration Projects</h2>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {PROJECTS.map((p, i) => {
              ...
              return (
                <tr key={rowKey} className="border-t border-gray-100 hover:bg-gray-50/70">
                  <td className="px-5 py-3.5 font-medium text-slate-900">{p.name}</td>
                  <td className="px-3 py-3.5">
                    <span className="rounded-md bg-gray-100 px-2 py-0.5 text-xs font-medium text-slate-500">{p.object}</span>
                  </td>
                  <td className="px-3 py-3.5 text-slate-700">{p.source}</td>
                  <td className="px-3 py-3.5">
                    <span className="rounded-md bg-gray-50 px-2 py-0.5 text-xs text-slate-500">{p.method}</span>
                  </td>
                  <td className="px-3 py-3.5 text-slate-700">{p.batches} Batches</td>
