...
def _load_key_file_dataframe(
    ...
    """Load key-detection rows from PostgreSQL first; local disk is fallback only."""
    if not file_ref:
        return None
    ...
    if DATABASE_URL:
        try:
            conn = _get_db_conn()
            db_rows = _load_key_rows_from_dynamic_table(
                conn, file_ref, sheet_name=sheet_name, target_column=target_column
            )
...
def _load_key_rows_from_dynamic_table(
    conn,
    file_ref: str,
    sheet_name: Optional[str] = None,
    target_column: Optional[str] = None,
) -> Optional[pd.DataFrame]:
    """Read source/target/FBDI rows from the dynamic PostgreSQL tables."""
    ...
    try:
        cur.execute(
            ...
            WHERE dt.file_id = %s
               OR af.storage_path = %s OR af.file_name = %s
               OR ff.storage_path = %s OR ff.file_name = %s
            ORDER BY CASE WHEN %s IS NOT NULL AND dt.sheet_name = %s THEN 0 ELSE 1 END,
                     dt.created_at DESC
            """,
            (
                file_ref, file_ref, os.path.basename(file_ref),
                file_ref, os.path.basename(file_ref), sheet_name, sheet_name,
            ),
        )
        ...
        rename_map = {
            ...
            if column.get("pg_name") and column.get("name")
        }
        return df.rename(columns=rename_map)
    except Exception:
        # Older databases may not have dynamic-table registry rows yet.
        return None
    finally:
        cur.close()
...
def validate_custom_key_endpoint(request: CustomKeyValidationRequest):
    try:
        df_src = _load_key_file_dataframe(request.source_file)
        if df_src is None:
...
def evaluate_key_pair(request: KeyPairEvaluationRequest):
    try:
        df_src = _load_key_file_dataframe(request.source_file)
        df_tgt = _load_key_file_dataframe(request.target_file)
